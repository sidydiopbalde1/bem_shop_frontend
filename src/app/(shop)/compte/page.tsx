    ...
    <div style={{ marginBottom:'14px' }}>
      ...
      <input name={name} type={type} value={value} autoComplete={autoComplete} required
        ...
        style={{
          ...
          fontSize:'16px', color:'var(--bem-black)', outline:'none',
...
function SubmitBtn({ busy, label, loadingLabel }: { busy:boolean; label:string; loadingLabel:string }) {
  return (
    <motion.button type="submit" disabled={busy} whileTap={{ scale:0.98 }}
      style={{
        width:'100%', height:'54px', borderRadius:'12px', border:'none',
        cursor: busy ? 'wait' : 'pointer',
        background: busy ? 'var(--bem-gray-400)' : 'var(--bem-black)',
        color:'#fff', fontWeight:800, fontSize:'14px', letterSpacing:'.08em',
  ...
  return (
    <button type="button" onClick={() => { window.location.href = `${BACKEND}/auth/google`; }}
      style={{
        ...
        fontSize:'14px', fontWeight:600, color:'var(--bem-black)', cursor:'pointer',
...
function LoginForm({ onSwitch }: { onSwitch:()=>void }) {
  ...
  return (
    <form onSubmit={handle}>
      ...
      <p style={{ textAlign:'center', fontSize:'13px', color:'var(--bem-gray-400)', marginTop:'18px' }}>
...
function RegisterForm({ onSwitch }: { onSwitch:()=>void }) {
  ...
  return (
    <form onSubmit={handle}>
      {error && <ErrorBox msg={error} />}
      {/* Grille responsive : 2 colonnes sur sm+, 1 colonne sur mobile */}
  ...
  return (
    <div className="auth-layout">
        ...
        <div>
          ...
          <Fade delay={120}>
            <p style={{ color:'rgba(255,255,255,.5)', fontSize:'13px', lineHeight:1.7, maxWidth:'300px' }}>
              Accédez à vos commandes, gérez votre profil et retrouvez tous vos achats.
            </p>
          </Fade>
          <Fade delay={160}>
      ...
      <div style={{
        ...
        background:'#fafaf9', overflowY:'auto',
        padding:'clamp(1.5rem, 5vw, 3rem) clamp(1rem, 5vw, 2rem)',
        paddingBottom:'calc(clamp(1.5rem, 5vw, 3rem) + 80px)',
      }}>
        <div style={{ width:'100%', maxWidth:'400px', paddingTop:'8px' }}>
          ...
          <Fade>
            <h2 style={{ fontFamily:'var(--font-playfair),serif', fontSize:'clamp(1.5rem, 5vw, 1.75rem)', fontWeight:800, color:'var(--bem-black)', marginBottom:'5px' }}>
              {tab==='login' ? 'Bon retour !' : 'Rejoignez-nous'}
            </h2>
            <p style={{ color:'var(--bem-gray-400)', fontSize:'14px', marginBottom:'24px' }}>
          ...
          {/* Tabs */}
          <Fade delay={40}>
            <div style={{ display:'flex', gap:'3px', padding:'4px', background:'var(--bem-gray-100)', borderRadius:'12px', marginBottom:'22px' }}>
              {(['login','register'] as const).map((t) => (
                <button key={t} type="button" onClick={() => setTab(t)} style={{
                  flex:1, height:'42px', border:'none', cursor:'pointer', borderRadius:'9px',
                  fontSize:'13px', fontWeight:600, transition:'all .18s',
          ...
          <Fade delay={70}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', margin:'18px 0' }}>
              ...
              <span style={{ fontSize:'12px', color:'var(--bem-gray-400)', fontWeight:500 }}>ou</span>
              <div style={{ flex:1, height:'1px', background:'var(--bem-gray-100)' }} />
            </div>
          </Fade>
          ...
          <Fade delay={120}>
            <p style={{ textAlign:'center', fontSize:'12px', color:'var(--bem-gray-400)', marginTop:'24px' }}>
...
function OrderCard({ order }: { order:Order }) {
  ...
  return (
    ...
    >
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', borderBottom:'1px solid var(--bem-gray-100)', flexWrap:'wrap', gap:'8px' }}>
        <div>
      ...
      <div style={{ padding:'14px 18px', display:'flex', alignItems:'center', gap:'10px' }}>
        <div style={{ display:'flex', gap:'5px', flexShrink:0 }}>
          {order.items.slice(0,3).map((item) => (
            <div key={item.id} style={{ width:'46px', height:'46px', borderRadius:'8px', overflow:'hidden', background:'var(--bem-gray-50)', border:'1px solid var(--bem-gray-100)', position:'relative', flexShrink:0 }}>
              {item.product.imageUrls?.[0]
                ? <Image src={item.product.imageUrls[0]} alt={item.product.name} fill className="object-cover" sizes="46px" />
                : <div style={{ width:'100%', height:'100%', background:'var(--bem-gray-100)' }} />}
            </div>
          ))}
          {order.items.length > 3 && (
            <div style={{ width:'46px', height:'46px', borderRadius:'8px', background:'var(--bem-gray-50)', border:'1px solid var(--bem-gray-100)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:700, color:'var(--bem-gray-400)' }}>
              +{order.items.length-3}
            </div>
          )}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ fontSize:'12px', fontWeight:600, lineHeight:1.3, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
...
function OrdersSection({ onBack }: { onBack:()=>void }) {
  ...
  return (
    <div style={{ maxWidth:'860px', margin:'0 auto', padding:'28px clamp(16px, 4vw, 48px) calc(80px + 24px)' }}>
      <button onClick={onBack}
        style={{ display:'inline-flex', alignItems:'center', gap:'8px', marginBottom:'24px', background:'none', border:'none', cursor:'pointer', fontSize:'13px', fontWeight:600, color:'var(--bem-gray-400)', padding:0, transition:'color .15s', minHeight:'44px' }}
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'20px', flexWrap:'wrap', gap:'12px' }}>
        <div>
          <h2 style={{ fontSize:'20px', fontWeight:900 }}>Mes commandes</h2>
          {!loading && !error && <p style={{ fontSize:'12px', color:'var(--bem-gray-400)', marginTop:'3px' }}>{orders.length} commande{orders.length!==1?'s':''}</p>}
        </div>
        <button onClick={fetch_}
          style={{ display:'inline-flex', alignItems:'center', gap:'6px', padding:'10px 16px', borderRadius:'99px', border:'1.5px solid var(--bem-gray-100)', background:'#fff', cursor:'pointer', fontSize:'12px', fontWeight:600, color:'var(--bem-gray-400)', transition:'all .15s', minHeight:'44px' }}
      ...
      {!loading && error && (
        <div style={{ padding:'18px 20px', borderRadius:'14px', background:'#fef2f2', border:'1px solid #fecaca', display:'flex', alignItems:'center', gap:'12px' }}>
      ...
      {!loading && !error && orders.length===0 && (
        <div style={{ textAlign:'center', padding:'52px 20px', background:'#fff', borderRadius:'16px', border:'1px solid var(--bem-gray-100)' }}>
          <div style={{ width:'60px', height:'60px', borderRadius:'50%', background:'var(--bem-gray-50)', border:'2px solid var(--bem-gray-100)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
          ...
          <p style={{ fontSize:'13px', color:'var(--bem-gray-400)', marginBottom:'22px' }}>Vos commandes apparaîtront ici après votre premier achat.</p>
          <Link href="/catalogue" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'12px 24px', borderRadius:'99px', background:'var(--bem-black)', color:'#fff', fontSize:'13px', fontWeight:700, textDecoration:'none' }}>
            Voir le catalogue
          </Link>
        </div>
      )}
...
function ProfileSection({ onBack }: { onBack:()=>void }) {
  ...
  const inputStyle: React.CSSProperties = { width:'100%', height:52, padding:'0 14px', borderRadius:12, border:'1.5px solid var(--bem-gray-100)', background:'var(--bem-gray-50)', fontSize:16, color:'var(--bem-black)', outline:'none', boxSizing:'border-box', transition:'border-color .18s', WebkitAppearance:'none' };
  const labelStyle: React.CSSProperties = { display:'block', fontSize:10, fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', color:'var(--bem-gray-400)', marginBottom:7 };

  return (
    <div style={{ maxWidth:'620px', margin:'0 auto', padding:'28px clamp(16px, 4vw, 48px) calc(80px + 24px)' }}>
      <button onClick={onBack} style={{ display:'inline-flex', alignItems:'center', gap:8, marginBottom:24, background:'none', border:'none', cursor:'pointer', fontSize:13, fontWeight:600, color:'var(--bem-gray-400)', padding:0, transition:'color .15s', minHeight:'44px' }}
      ...
      <h2 style={{ fontSize:22, fontWeight:900, marginBottom:20 }}>Mon profil</h2>

      <div style={{ background:'#fff', borderRadius:16, border:'1px solid var(--bem-gray-100)', padding:'20px', marginBottom:14 }}>
        <p style={{ fontSize:13, fontWeight:700, marginBottom:16 }}>Informations personnelles</p>
        {msg && (
          <div style={{ padding:'10px 14px', borderRadius:9, marginBottom:14, fontSize:12, fontWeight:600, background: msg.type==='ok' ? 'rgba(22,163,74,.1)' : 'rgba(204,31,39,.08)', color: msg.type==='ok' ? '#15803d' : 'var(--bem-red)', border: `1px solid ${msg.type==='ok' ? 'rgba(22,163,74,.2)' : 'rgba(204,31,39,.2)'}` }}>
            {msg.text}
          </div>
        )}
        ...
        <form onSubmit={handleProfile}>
          ...
          <button type="submit" disabled={busy} style={{ height:48, padding:'0 24px', borderRadius:12, border:'none', background:busy?'var(--bem-gray-400)':'var(--bem-black)', color:'#fff', fontWeight:700, fontSize:14, cursor:busy?'wait':'pointer', transition:'background .2s', WebkitAppearance:'none' }}>
            {busy ? 'Sauvegarde…' : 'Sauvegarder'}
          </button>
        </form>
      </div>

      <div style={{ background:'#fff', borderRadius:16, border:'1px solid var(--bem-gray-100)', padding:'20px' }}>
        <p style={{ fontSize:13, fontWeight:700, marginBottom:16 }}>Changer le mot de passe</p>
        {pwMsg && (
          <div style={{ padding:'10px 14px', borderRadius:9, marginBottom:14, fontSize:12, fontWeight:600, background: pwMsg.type==='ok' ? 'rgba(22,163,74,.1)' : 'rgba(204,31,39,.08)', color: pwMsg.type==='ok' ? '#15803d' : 'var(--bem-red)', border: `1px solid ${pwMsg.type==='ok' ? 'rgba(22,163,74,.2)' : 'rgba(204,31,39,.2)'}` }}>
            {pwMsg.text}
          </div>
        )}
        <form onSubmit={handlePassword} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {[
            ...
            <div key={key}>
              ...
              <input type="password" style={inputStyle} value={pwForm[key as keyof typeof pwForm]} required minLength={key==='currentPassword'?1:8}
                onChange={(e)=>setPwForm(f=>({...f,[key]:e.target.value}))}
                onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/>
            </div>
          ))}
          <button type="submit" disabled={pwBusy} style={{ height:48, padding:'0 24px', borderRadius:12, border:'none', background:pwBusy?'var(--bem-gray-400)':'var(--bem-black)', color:'#fff', fontWeight:700, fontSize:14, cursor:pwBusy?'wait':'pointer', WebkitAppearance:'none' }}>
...
function AddressForm({ initial, onSave, onCancel, busy }: { initial: typeof ADDR_EMPTY; onSave:(d:typeof ADDR_EMPTY)=>void; onCancel:()=>void; busy:boolean }) {
  ...
  return (
    <form onSubmit={(e)=>{ e.preventDefault(); onSave(form); }} style={{ display:'flex', flexDirection:'column', gap:12 }}>
      <div><label style={labelStyle}>Libellé *</label><input style={inputStyle} placeholder="Ex: Maison, Bureau…" value={form.label} required onChange={(e)=>s('label')(e.target.value)} onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/></div>
      <div><label style={labelStyle}>Rue / Adresse *</label><input style={inputStyle} placeholder="Ex: Rue 10, Liberté 6" value={form.street} required onChange={(e)=>s('street')(e.target.value)} onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><label style={labelStyle}>Ville *</label><input style={inputStyle} placeholder="Dakar" value={form.city} required onChange={(e)=>s('city')(e.target.value)} onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/></div>
        <div><label style={labelStyle}>Quartier</label><input style={inputStyle} placeholder="Plateau" value={form.district} onChange={(e)=>s('district')(e.target.value)} onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/></div>
      </div>
      <div><label style={labelStyle}>Téléphone</label><input style={inputStyle} placeholder="+221 77 000 00 00" value={form.phone} onChange={(e)=>s('phone')(e.target.value)} onFocus={(e)=>(e.currentTarget.style.borderColor='var(--bem-black)')} onBlur={(e)=>(e.currentTarget.style.borderColor='var(--bem-gray-100)')}/></div>
      <div style={{ display:'flex', gap:8, paddingTop:4 }}>
        <button type="button" onClick={onCancel} style={{ flex:1, height:48, borderRadius:12, border:'1.5px solid var(--bem-gray-100)', background:'#fff', fontSize:14, fontWeight:600, cursor:'pointer', WebkitAppearance:'none' }}>Annuler</button>
        <button type="submit" disabled={busy} style={{ flex:2, height:48, borderRadius:12, border:'none', background:busy?'var(--bem-gray-400)':'var(--bem-black)', color:'#fff', fontSize:14, fontWeight:700, cursor:busy?'wait':'pointer', WebkitAppearance:'none' }}>
          {busy ? 'Sauvegarde…' : 'Sauvegarder'}
        </button>
      </div>
    </form>
  ...
  return (
    <div style={{ maxWidth:'620px', margin:'0 auto', padding:'28px clamp(16px, 4vw, 48px) calc(80px + 24px)' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        {!adding && !editId && (
          <button onClick={()=>setAdding(true)} style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'10px 18px', borderRadius:99, background:'var(--bem-black)', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', minHeight:'44px', WebkitAppearance:'none' }}>
      ...
      {error && (
        <div style={{ padding:'12px 16px', borderRadius:10, marginBottom:14, fontSize:12, fontWeight:600, background:'rgba(204,31,39,.08)', color:'var(--bem-red)', border:'1px solid rgba(204,31,39,.2)', display:'flex', justifyContent:'space-between' }}>
          {error}<button onClick={()=>setError(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--bem-red)', fontSize:16 }}>×</button>
        </div>
      )}

      {adding && (
        <div style={{ background:'#fff', borderRadius:16, border:'1.5px solid var(--bem-black)', padding:20, marginBottom:14 }}>
          <p style={{ fontSize:13, fontWeight:700, marginBottom:14 }}>Nouvelle adresse</p>
          <AddressForm initial={ADDR_EMPTY} onSave={handleAdd} onCancel={()=>setAdding(false)} busy={formBusy} />
        </div>
      )}

      {loading && <div style={{ display:'flex', flexDirection:'column', gap:10 }}>{[1,2].map(i=><div key={i} style={{ height:100, borderRadius:14, background:'var(--bem-gray-100)', animation:'shimmer 1.4s ease infinite' }}/>)}</div>}
      ...
      {!loading && addresses.length===0 && !adding && (
        <div style={{ textAlign:'center', padding:'48px 20px', background:'#fff', borderRadius:16, border:'1px solid var(--bem-gray-100)' }}>
          <div style={{ width:52, height:52, borderRadius:'50%', background:'var(--bem-gray-50)', border:'2px solid var(--bem-gray-100)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px' }}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--bem-gray-400)" strokeWidth="1.4"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 1 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
          </div>
          ...
          <button onClick={()=>setAdding(true)} style={{ padding:'12px 24px', borderRadius:99, background:'var(--bem-black)', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', WebkitAppearance:'none' }}>Ajouter une adresse</button>
        </div>
      )}

      {!loading && addresses.map((addr) => (
        <div key={addr.id} style={{ background:'#fff', borderRadius:14, border:`1.5px solid ${addr.isDefault?'var(--bem-black)':'var(--bem-gray-100)'}`, padding:'16px 18px', marginBottom:10, opacity:actionId===addr.id?0.5:1, transition:'all .15s' }}>
          {editId===addr.id ? (
            <><p style={{ fontSize:13, fontWeight:700, marginBottom:14 }}>Modifier l&apos;adresse</p>
            <AddressForm initial={{ label:addr.label, street:addr.street, city:addr.city, district:addr.district??'', phone:addr.phone??'' }} onSave={handleEdit} onCancel={()=>setEditId(null)} busy={formBusy} /></>
          ) : (
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10 }}>
              <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                <div style={{ width:36, height:36, borderRadius:9, background:addr.isDefault?'var(--bem-black)':'var(--bem-gray-50)', color:addr.isDefault?'#fff':'var(--bem-gray-400)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 1 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                </div>
                <div>
                  ...
                  <p style={{ fontSize:12, color:'var(--bem-gray-700)', lineHeight:1.5 }}>{addr.street}{addr.district?`, ${addr.district}`:''}, {addr.city}</p>
                  {addr.phone && <p style={{ fontSize:12, color:'var(--bem-gray-400)', marginTop:2 }}>{addr.phone}</p>}
                </div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:6, flexShrink:0 }}>
                {!addr.isDefault && <button onClick={()=>handleSetDefault(addr.id)} disabled={!!actionId} style={{ padding:'6px 10px', borderRadius:8, border:'1.5px solid var(--bem-gray-100)', background:'#fff', fontSize:11, fontWeight:600, cursor:'pointer', color:'var(--bem-gray-700)', minHeight:'36px', WebkitAppearance:'none' }}>Défaut</button>}
                <button onClick={()=>setEditId(addr.id)} disabled={!!actionId} style={{ padding:'6px 10px', borderRadius:8, border:'none', background:'rgba(37,99,235,.1)', fontSize:11, fontWeight:600, cursor:'pointer', color:'#1d4ed8', minHeight:'36px', WebkitAppearance:'none' }}>Modifier</button>
                <button onClick={()=>handleDelete(addr.id)} disabled={!!actionId} style={{ padding:'6px 10px', borderRadius:8, border:'none', background:'rgba(204,31,39,.1)', fontSize:11, fontWeight:600, cursor:'pointer', color:'var(--bem-red)', minHeight:'36px', WebkitAppearance:'none' }}>Supprimer</button>
              </div>
            </div>
          )}
        </div>
...
function Dashboard() {
  ...
  if (section==='orders')    return <div style={{ background:'#fafaf9', minHeight:'100dvh' }}><OrdersSection    onBack={()=>setSection('home')} /></div>;
  if (section==='profile')   return <div style={{ background:'#fafaf9', minHeight:'100dvh' }}><ProfileSection   onBack={()=>setSection('home')} /></div>;
  if (section==='addresses') return <div style={{ background:'#fafaf9', minHeight:'100dvh' }}><AddressesSection onBack={()=>setSection('home')} /></div>;
  ...
  return (
    <div style={{ background:'#fafaf9', minHeight:'100dvh' }}>
      {/* Hero */}
      <div style={{ background:'var(--bem-black)', padding:'32px clamp(16px, 4vw, 48px) 40px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:'860px', margin:'0 auto', position:'relative' }}>
          <Fade>
            <p style={{ fontSize:'10px', fontWeight:700, letterSpacing:'.18em', textTransform:'uppercase', color:'var(--bem-red)', marginBottom:'16px' }}>
              Mon espace
            </p>
          </Fade>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:'16px', flexWrap:'wrap' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'16px' }}>
              <Fade delay={30}>
                <div style={{ width:'clamp(52px, 10vw, 68px)', height:'clamp(52px, 10vw, 68px)', borderRadius:'50%', background:'var(--bem-red)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'clamp(16px, 4vw, 22px)', fontWeight:800, flexShrink:0, boxShadow:'0 0 0 3px rgba(204,31,39,.3)' }}>
                  {initials(user.firstName,user.lastName)}
                </div>
              </Fade>
              <div>
                <Fade delay={60}>
                  <h1 style={{ fontFamily:'var(--font-playfair),serif', fontSize:'clamp(1.2rem,4vw,2rem)', fontWeight:800, color:'#fff', lineHeight:1.2 }}>
            ...
            <Fade delay={110}>
              <button onClick={async()=>{setLoggingOut(true);await logout();}} disabled={loggingOut}
                style={{ display:'flex', alignItems:'center', gap:'8px', padding:'10px 18px', borderRadius:'99px', background:'rgba(255,255,255,.07)', border:'1px solid rgba(255,255,255,.14)', color:'rgba(255,255,255,.6)', fontSize:'12px', fontWeight:600, cursor:loggingOut?'wait':'pointer', transition:'all .18s', minHeight:'44px', WebkitAppearance:'none' }}
      ...
      {/* Content */}
      <div style={{ maxWidth:'860px', margin:'0 auto', padding:'24px clamp(16px, 4vw, 48px) calc(80px + 24px)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:'10px', marginBottom:'24px' }}>
          {[
            ...
            <Fade key={s.label} delay={130+i*30}>
              <div style={{ background:'#fff', borderRadius:'14px', border:'1px solid var(--bem-gray-100)', padding:'16px clamp(12px, 3vw, 20px)' }}>
                <p style={{ fontSize:'clamp(20px, 5vw, 28px)', fontWeight:900, lineHeight:1 }}>{s.val}</p>
                <p style={{ fontSize:'clamp(9px, 2.5vw, 11px)', color:'var(--bem-gray-400)', marginTop:'5px' }}>{s.label}</p>
              </div>
            </Fade>
          ))}
        </div>
        ...
        {/* Nav cards */}
        <p style={{ fontSize:'10px', fontWeight:700, letterSpacing:'.14em', textTransform:'uppercase', color:'var(--bem-gray-400)', marginBottom:'12px' }}>
          Accès rapide
        </p>
        <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            ...
            <Fade key={item.label} delay={230+i*35}>
              <button onClick={item.enabled?item.action:undefined}
                ...
                style={{
                  ...
                  border:'1px solid var(--bem-gray-100)', padding:'16px 18px',
                  display:'flex', alignItems:'center', gap:'14px',
                  cursor: item.enabled?'pointer':'default',
                  opacity: item.enabled?1:.45,
                  transition:'all .18s', minHeight:'72px',
                  WebkitAppearance:'none',
                }}
                onMouseEnter={(e)=>{ if (!item.enabled) return; const el=e.currentTarget as HTMLElement; el.style.borderColor=item.accent; el.style.boxShadow='0 4px 20px rgba(0,0,0,.08)'; }}
                onMouseLeave={(e)=>{ const el=e.currentTarget as HTMLElement; el.style.borderColor='var(--bem-gray-100)'; el.style.boxShadow='none'; }}
              >
                <div style={{ width:'44px', height:'44px', borderRadius:'12px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:`${item.accent}18`, color:item.accent }}>
                  {item.icon}
                </div>
                <div style={{ flex:1 }}>
                  <p style={{ fontWeight:700, fontSize:'14px' }}>{item.label}</p>
                  <p style={{ fontSize:'12px', color:'var(--bem-gray-400)', marginTop:'3px' }}>{item.sub}</p>
                </div>
                {item.enabled && (
                  <div style={{ width:'28px', height:'28px', borderRadius:'50%', background:'var(--bem-gray-50)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--bem-gray-400)" strokeWidth="2.5">
        ...
        {/* Promo */}
        <Fade delay={340}>
          <div style={{ marginTop:'24px', padding:'24px clamp(16px, 4vw, 32px)', background:'var(--bem-black)', borderRadius:'18px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'16px', flexWrap:'wrap', position:'relative', overflow:'hidden' }}>
            <div style={{ position:'absolute', right:'-30px', top:'-30px', width:'140px', height:'140px', borderRadius:'50%', background:'rgba(204,31,39,.12)', pointerEvents:'none' }} />
            <div>
            ...
            <Link href="/catalogue" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'11px 20px', borderRadius:'99px', background:'var(--bem-red)', color:'#fff', fontWeight:700, fontSize:'13px', textDecoration:'none', flexShrink:0, boxShadow:'0 4px 16px rgba(204,31,39,.4)', minHeight:'44px' }}>
              Voir le catalogue →
            </Link>
          </div>
        </Fade>
...
function Skeleton() {
  return (
    <div>
      <div style={{ background:'var(--bem-black)', height:'160px' }} />
      <div style={{ maxWidth:'860px', margin:'0 auto', padding:'24px clamp(16px, 4vw, 48px)' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:24 }}>
          {[1,2,3].map((i)=><div key={i} style={{ height:72, borderRadius:14, background:'var(--bem-gray-100)', animation:'shimmer 1.4s ease infinite', animationDelay:`${i*.1}s` }} />)}
        </div>
...
export default function ComptePage() {
  ...
  useEffect(() => {
    ...
    if (user.role === 'ADMIN') { router.replace('/admin/analytics'); return; }
